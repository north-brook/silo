...
sed -i 's/^ZSH_THEME=.*/ZSH_THEME="robbyrussell"/' /etc/skel/.zshrc
grep -qxF 'export PATH="/usr/local/bin:$PATH"' /etc/skel/.zshrc || \
  printf '\nexport PATH="/usr/local/bin:$PATH"\n' >> /etc/skel/.zshrc
grep -qxF '[[ -f /etc/profile.d/silo-homebrew.sh ]] && source /etc/profile.d/silo-homebrew.sh' /etc/skel/.zshrc || \
