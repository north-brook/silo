...
const SessionHostContext = createContext<SessionHostContextValue | null>(null);
...
export function SessionHostProvider({
	children,
	...
	const contextValue = useMemo<SessionHostContextValue>(
		() => ({
			activeSessionKey,
			activeWorkspace,
			ensureSession,
	...
	return (
		<SessionHostContext.Provider value={contextValue}>
			{children}
...
export function useSessionHosts() {
	const context = useContext(SessionHostContext);
	if (!context) {
		throw new Error(
			"useSessionHosts must be used within a SessionHostProvider",
		);
	}
	return context;
}
